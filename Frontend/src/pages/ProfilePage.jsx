// pages/ProfilePage.jsx
import { useState, useEffect } from "react";
import { useNavigate } from "react-router-dom";
import { useUser } from "../hooks/useUser";
import { useAuth } from "../hooks/useAuth";
import Navbar from "../components/Navbar";
import ProfileImage from "../components/ProfileImage";
import ProfileProgress from "../components/ProfileProgress";
import ProfileForm from "../components/ProfileForm";
import { validateProfile } from "../utils/profileValidation";
import { useTranslation } from "react-i18next";
import toast from "react-hot-toast";
import Swal from "sweetalert2";

export default function ProfilePage() {
  const navigate = useNavigate();
  const { t } = useTranslation();
  const { profile, loading, error, success, fetchMyInfo, updateProfile, clearState, removeAccount } = useUser();

  const FIELD_KEYS = ["firstName", "lastName", "email", "phone", "age", "nationality", "city"];
  
  const calcProgress = (data) => {
    const filled = FIELD_KEYS.filter((k) => String(data[k] || "").trim() !== "").length;
    return Math.round((filled / FIELD_KEYS.length) * 100);
  };

  const [formData, setFormData] = useState({});
  const [profileImage, setProfileImage] = useState("images/default-avatar.jpeg");
  const [cvFile, setCvFile] = useState(null);
  const [localImage, setLocalImage] = useState(null);

  // ── جلب بيانات المستخدم عند تحميل الصفحة وتنظيف الـ State عند مغادرتها ──
  useEffect(() => {
    fetchMyInfo();
    return () => {
      if (clearState) clearState();
    };
  }, []);

  // ── دمج ومزامنة بيانات البروفايل القادمة من الـ API مع الـ Form المحلية ──
  useEffect(() => {
    if (!profile) return;
    setFormData({
      firstName: profile.firstName || "",
      lastName: profile.lastName || "",
      email: profile.email || "",
      phone: profile.phone || "",
      age: profile.age || "",
      nationality: profile.nationality || "",
      city: profile.city || "",
    });
    if (profile.profileImage) setProfileImage(profile.profileImage);
  }, [profile]);

  // ── التعامل مع نجاح عملية التحديث ──
  useEffect(() => {
    if (success) {
      toast.success(t('profile.update_success'));
      if (clearState) clearState();
    }
  }, [success, t]);

  // ── معالجة التغييرات على الحقول والملفات ──
  const handleFieldChange = (key, value) => setFormData((prev) => ({ ...prev, [key]: value }));
  const handleImageChange = (base64) => { setLocalImage(base64); setProfileImage(base64); };
  const handleCvChange = (e) => setCvFile(e.target.files[0] || null);

  // ── حفظ البيانات بعد التحقق من صحتها ──
  const handleSave = () => {
    const userDto = { ...formData };
    if (localImage) userDto.profileImage = localImage;

    const { isValid, errors } = validateProfile(userDto);
    if (!isValid) { 
      errors.forEach((err) => toast.error(err)); 
      return; 
    }

    updateProfile(userDto, cvFile);
  };

  // ── حذف الحساب مع إظهار نافذة تأكيد احترافية متكاملة ──
  const handleDeleteAccount = () => {
    Swal.fire({
      title: t('profile.delete_confirm_title'),
      text: t('profile.delete_confirm_text'),
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#1a9e8f",
      cancelButtonColor: "#d33",
      confirmButtonText: t('profile.delete_confirm_btn'),
      cancelButtonText: t('profile.delete_cancel_btn'),
    }).then(async (result) => {
      if (!result.isConfirmed) return;
      
      try {
        Swal.fire({ 
          title: t('profile.delete_loading_title'), 
          text: t('profile.delete_loading_text'), 
          allowOutsideClick: false, 
          didOpen: () => { Swal.showLoading(); } 
        });
        
        await removeAccount();
        localStorage.clear();
        
        await Swal.fire({ 
          title: t('profile.delete_success_title'), 
          text: t('profile.delete_success_text'), 
          icon: "success", 
          timer: 1200, 
          showConfirmButton: false 
        });
        
        navigate("/register");
      } catch (err) {
        Swal.fire({ 
          title: t('profile.delete_error_title'), 
          text: err || t('profile.delete_error_fallback'), 
          icon: "error" 
        });
      }
    });
  };

  return (
    <>
      <Navbar />

      <div className="profile-page-wrapper">
        <div className="container py-3">
          <div className="profile-page-container shadow-sm rounded-4 p-4 p-lg-5 border border-light">
            
            {/* Header Section */}
            <div className="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center mb-4 pb-3 border-bottom gap-3">
              <h3 className="fw-bold mb-0">{t('profile.page_title')}</h3>
              <button 
                type="button" 
                className="delete-account-btn btn btn-outline-danger rounded-pill px-4 fw-semibold shadow-sm d-inline-flex align-items-center justify-content-center gap-2" 
                onClick={handleDeleteAccount}
              >
                <i className="fa-solid fa-user-xmark"></i> {t('profile.delete_account_btn')}
              </button>
            </div>

            {/* Error Notification */}
            {error && (
              <div className="alert alert-danger rounded-3 border-0 shadow-sm d-flex align-items-center mb-4">
                <i className="fa-solid fa-circle-exclamation fs-5 me-2"></i>
                {error}
              </div>
            )}

            {/* Layout Main Grid */}
            <div className="row g-5">
              <div className="col-md-4 d-flex flex-column align-items-center border-end-md border-light">
                <ProfileImage src={profileImage} onImageChange={handleImageChange} />
                <ProfileProgress percent={calcProgress(formData)} />
              </div>
              <div className="col-md-8">
                <ProfileForm 
                  formData={formData} 
                  onChange={handleFieldChange} 
                  onSave={handleSave} 
                  onCvChange={handleCvChange} 
                  loading={loading} 
                />
              </div>
            </div>

          </div>
        </div>
      </div>
    </>
  );
}